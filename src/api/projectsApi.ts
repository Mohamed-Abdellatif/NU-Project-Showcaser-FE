...
/**
 * Get related projects by project ID
 * @param id - MongoDB ObjectId of the project
 * @returns Array of related projects
 */
export const getRelatedProjects = async (id: string): Promise<Project[]> => {
  try {
    const response = await axios.get<Project[]>(`${API_BASE}/related/${id}`, {
      withCredentials: true,
    });
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to get related projects"
      );
    }
    throw error;
  }
